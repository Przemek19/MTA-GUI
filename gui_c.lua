...
gui.config.defaultColor = {
  primary = gui.color.default,
  background = {30,32,36,160},
  text = gui.color.white,
  border = gui.color.default,
}
...
addEventHandler("onClientResourceStart", getRootElement(), function(resource)
  ...
  addEventHandler("on" .. gui.config.functionsPrefix .. "BrowserCreated", createEditBox(-1,-1,1,1), function()
    source:destroy()
  end)
end)
